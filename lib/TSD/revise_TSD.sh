...
grep -B 1 KO ${NAME_FILE}_KO.txt | grep ">" | tr -d ">" | awk -F":" -v s_fk="$SIZE_FLANK" 'OFS="\t" {print $1, $3-s_fk, $3, $5":FLANK_LEFT\n"$1, $3, $3+s_fk, $5":FLANK_RIGHT"}' > ${WORK_TMP_DIR}/empty_site_KO.bed

bedtools getfasta -fi $GENOME -bed ${WORK_TMP_DIR}/empty_site_KO.bed -name+ > ${WORK_TMP_DIR}/empty_site_KO.fasta

echo ${NAME_FILE}_KO_corrected.txt;
python3 `dirname $0`/revise_TSD.py ${WORK_TMP_DIR}/empty_site_KO.fasta ${NAME_FILE}_KO.txt $SIZE_KMER > ${NAME_FILE}_KO_corrected.txt
...
grep -A 1 OK $FILE_TSD | grep ">" | tr -d ">" | awk -F":" '{print substr($1, length($1)-5, 2)""substr($2, 1, 6)}' > ${WORK_TMP_DIR}/TSM_OK.txt
grep -o "SVI=[A-Z]*" ${NAME_FILE}_KO_corrected.txt | cut -d"=" -f 2 >> ${WORK_TMP_DIR}/TSM_OK.txt

#OK
grep -A 2 -B 2 OK $FILE_TSD > ${NAME_FILE}_OK.txt 
grep -B 1 OK ${NAME_FILE}_OK.txt | grep ">" | tr -d ">" | awk -F":" -v s_fk="$SIZE_FLANK" 'OFS="\t" {print $1, $3-s_fk, $3, $5":FLANK_LEFT\n"$1, $3, $3+s_fk, $5":FLANK_RIGHT"}' > ${WORK_TMP_DIR}/empty_site_OK.bed
