...
ggplot(data, aes(x, y, fill=z, height=1), width=50) + 
  ...
  theme(axis.text.x = element_blank(),
          axis.text.y = element_text(family="Helvetica", face="plain", color="#222222", 
                           size=12, angle=0))+
  coord_fixed()+
  ...
  theme(text=element_text(family="Helvetica", size=14), plot.title = element_text(family="Hemondalisa", size=30, hjust = 0.5)) 
