...
<iframe src="lib/template_COUNT_TE_INOUTSIDER.html" style="width: 100%; height: 1000px; border: none; overflow: hidden;"></iframe>


### FREQUENCE

```{bash FREQB_inout, eval=TRUE, include=TRUE, results=TRUE}

test -s .tmp_ALL_TE_COUNT.csv && awk -F "\t" 'NR>1 {print $2}' .tmp_ALL_TE_COUNT.csv | tail -n 50 > .tmp_orderc.txt || echo
...
(test -s ../../INSIDER/FREQ_GLOBAL/DEPTH_TE_INSIDER.csv && awk 'NR>1' ../../INSIDER/FREQ_GLOBAL/DEPTH_TE_INSIDER.csv >> .tmp_DEPTH_TE.csv && python3 ../lib/freq_graph.py .tmp_DEPTH_TE.csv FREQ_TE.csv) || echo ;

(test -s FREQ_TE.csv && echo -e "x\ty\tcondition" > .tmp_depth.csv && grep -w -f .tmp_orderc.txt FREQ_TE.csv >> .tmp_depth.csv && mv .tmp_depth.csv FREQ_TE.csv) || echo

```



```{r FREQ_INOUT, eval=TRUE, include=TRUE, results=FALSE, out.width="90%", echo=FALSE}
